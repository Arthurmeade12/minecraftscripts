...
spigeteval(){
    intoupdate
    local FILE="$(find . -maxdepth 1 -name "${2}" 2>/dev/null)"
    local APIURL="https://api.spiget.org/v2/resources/${3}}/download"
    local NAME="${1^}"
    skipnohashupdate "${1}" || return 0
    curl -JLO# 
}
...
DATA
    ...
    then
        ...
        trash "${FILE}"
       	curl -JLOf# "$(mrdown "${APIURL}" | jq -r "${JQCOMMAND} | .url")"
        local FILE="$(find . -maxdepth 1 -name "${2}" 2>/dev/null)" # We need to redefine since filename has changed.
        if mrcheckintegrity
        then
            out "Updated ${1}"
        else
...
consturl 'CrossPlatforms' 'https://ci.kejonamc.dev/job/CrossplatForms/job/main/lastSuccessfulBuild/artifact/spigot/build/libs/CrossplatForms-Spigot.jar' # Jenkins, no way to get hash
...
warn 'Skipping Placeholder API!' # spigot.org
consturl 'ProtocolLib' 'https://ci.dmulloy2.net/job/ProtocolLib/lastSuccessfulBuild/artifact/build/libs/ProtocolLib.jar'
...
mreval 'Simple Voice Chat' 'voicechat-bukkit-*.jar' '8450fce54dc83a4c8fad0e9755999dd7b78530a9'
mreval 'Terra' 'Terra-*.jar' '3937d24a68a5efc9acfecbe67c78d026cd7ae5a8'
#warn "Skipping Terra, since we are using a custom build."
